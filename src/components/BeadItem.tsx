...
export default function BeadItem({ bead, onClick, isActive = false }: Props) {
  return (
    ...
    >
      <div
        className="w-10 h-10 rounded-full transition-transform duration-300 group-hover:scale-110 overflow-hidden"
        style={{
          backgroundColor: bead.imageDataUrl ? 'transparent' : bead.colorHex,
          boxShadow: bead.imageDataUrl
            ? 'none'
            : `0 0 12px ${bead.colorHex}60, inset 0 2px 3px rgba(255,255,255,0.25), inset 0 -2px 3px rgba(0,0,0,0.2)`,
